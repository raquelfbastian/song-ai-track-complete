...
export default function LabWorkspace({ lab }: { lab: LabNumber }) {
  ...
  return <div className="min-h-screen bg-stone-950 text-stone-100">
    <nav className="border-b border-stone-800 px-6 py-4 flex items-center justify-between gap-4">
      ...
      <div className="flex gap-3 text-xs text-stone-500">{[3, 4, 5, 6, 7, 8, 9, 10].map(number => <Link key={number} to={`/lab${number}`} className="hover:text-amber-400">L{number}</Link>)}</div>
    </nav>
